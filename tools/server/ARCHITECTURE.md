...
| `viewerStore.ts` | All selection/interaction state | State: `currentTimepoint`, `selectedGenes`, `selectedSpeciesNodes`, `selectedSpeciesTypes`, `selectedSegmentIds`, `hoveredModelPath`, `hoveredExecutionPath` (exposed). Computed: `activeModelPath` (hovered model takes priority, else derived from currentTimepoint + segments), `selectedPaths`, `proteinCountsAtTimepoint` (filters to hovered path or active-at-timepoint paths), `maxProteinCounts`. Actions: `selectSegments`, `selectExecutionPath(path)` (finds all segment IDs for that execution path and calls `selectSegments`), `setHoveredRectModel`, `setHoveredInstantModel` |
...
| File | Purpose |
| ------ | --------- |
| `MainChart.ts` | Orchestrates all panels. Manages two `PanelGroup`s (`timeseriesGroup`, `phaseSpaceGroup`) and a `ChartLayout` tree. Scoped modifiers only operate on `timeseriesGroup`. Phase-space API: `showPhaseSpace(result)`, `hidePhaseSpace()`, `setPhaseSpaceData(result)`, `setPhaseSpaceTimepoint(t)`, `onPhaseSpacePathSelect(cb)`, `onPhaseSpaceHover(cb)`. Path highlight: `highlightPath(path)` fans out to all panels. Callbacks: `onTimepointChange`, `onSelectionChange`, `onSegmentClick`, `onHoverChange`, `onTimeseriesPathHover(cb)`. |
| `panels/BasePanel.ts` | Abstract base: SciChartSubSurface, wasmContext, visibility, `setTimeExtent`, `highlightPath(path \| null)` (dims all series except those matching path via `<prefix>:<path>` naming convention; null restores all), `applyTheme`, `dispose` |
...
7. **Path highlight sync:** `viewerStore.hoveredExecutionPath` is the single hub. Writers: `TimelinePanel` hover, `PhaseSpaceHoverModifier`, `TimeseriesHoverModifier` (all via `setHoveredRectModel`). Reader: one watcher in `TrackViewer` calls `MainChart.highlightPath(path)` which fans out `BasePanel.highlightPath()` to every panel (dims non-matching series to 12% opacity). `TimelinePanel` overrides `highlightPath` to map `segment:<id>` names to execution paths via stored `rectangles`. `PhaseSpacePanel` overrides to skip when its own modifier is active. `PATH_DIM_OPACITY` exported from `BasePanel` for reuse.
8. Zoom in past threshold -> `AdaptiveZoom` swaps gene-scope edges for species-scope edges and adds species/reaction compound children (gene positions pinned) -> `ModelFilter.refresh()` + `SelectionSync.refresh()`.
